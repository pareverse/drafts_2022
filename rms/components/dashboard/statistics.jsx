...
const Statistics = () => {
	const data = [
		...
		{ id: 4, title: 'Total Collected', value: '₱' + (25000).toLocaleString(undefined, { maximumFractionDigits: 2 }), progress: 90, icon: <I.DollarSign /> }
	]

	return (
		<>
			{data.map((data) => (
				<C.GridItem colSpan={{ base: 12, lg: 6, xl: 3 }} key={data.id}>
					<Card p={6}>
						<C.Grid templateColumns="1fr 64px" gap={3}>
							...
							<C.GridItem>
								<C.Text fontSize="xs" fontWeight="medium" color="muted">
									{data.title}
								</C.Text>
							</C.GridItem>

							<C.GridItem colSpan={2}>
								<C.Progress colorScheme="blue" size="xs" borderRadius="md" value={data.progress} />
							</C.GridItem>
						</C.Grid>
					</Card>
				</C.GridItem>
